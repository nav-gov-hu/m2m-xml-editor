...
(()=>{
   ...
   */
const $=id=>document.getElementById(id); const query=new URLSearchParams(location.search); let items=[],activeCategory=(query.get('category')||'').toUpperCase(),advanced=query.get('mode')==='advanced',viewMode=query.get('view')==='card'?'card':'list',missingOnly=localStorage.getItem('m2mConfigurationMissingOnly')==='true',dirty=new Set(),sensitiveTouched=new Set(),searchTimer=null,pendingFocusKey='';
    /**
...
function listTable(data){return `<div class="configuration-list-wrap"><table class="configuration-list"><thead><tr><th>Beállítás</th><th>Érték</th><th>Tárolás</th><th>Forrás</th><th></th></tr></thead><tbody>${data.map(x=>`<tr class="configuration-list-row ${dirty.has(x.key)?'changed':''} ${proxyControlDisabled(x)?'configuration-dependent-disabled':''}" data-config-item="${esc(x.key)}"><td><strong>${esc(x.label)}${x.required?'<sup>*</sup>':''}</strong><p>${esc(x.description)}</p><code>${esc(x.key)}</code></td><td class="configuration-list-value">${control(x)}</td><td>${fieldTags(x)}</td><td><span>${esc(sourceLabel(x.source))}</span>${x.defaultValue!==''?`<small>Alapérték: ${esc(x.defaultValue)}</small>`:''}</td><td><button type="button" class="configuration-reset-link" data-reset-key="${esc(x.key)}">Visszaállítás</button></td></tr>`).join('')}</tbody></table></div>`;}
   ...
   * @returns {*} a feldolgozás eredménye
   */
function m2mItem(key){return items.find(item=>item.key===key);}
function m2mCompactField(key,label,extraClass=''){const item=m2mItem(key);if(!item)return '';return `<div class="m2m-connection-field ${extraClass} ${dirty.has(item.key)?'changed':''}" data-config-item="${esc(item.key)}"><div class="m2m-connection-field-head"><label>${esc(label)}${item.required?'<sup>*</sup>':''}</label><button type="button" class="configuration-reset-link" data-reset-key="${esc(item.key)}">Visszaállítás</button></div>${control(item)}<code>${esc(item.key)}</code></div>`;}
function m2mConnectionCardMarkup(){const keys=['nav.m2m.auth.username','nav.m2m.auth.password','nav.m2m.signature.key-first-part','nav.m2m.signature.nonce','nav.m2m.auth.client-id','nav.m2m.auth.client-secret'];if(!keys.some(key=>m2mItem(key)))return '';return `<section id="configuration-m2m-connection" class="configuration-tool-card m2m-connection-card" aria-labelledby="configurationM2mConnectionTitle"><div class="tool-card-head"><div><h3 id="configurationM2mConnectionTitle">M2M kapcsolat</h3><p>Az API kulcs négy része egy sorban adható meg. Teljes API kulcs beillesztésekor az első mező automatikusan feltölti mind a négy összetevőt.</p></div></div><div class="m2m-connection-subtitle">API kulcs</div><div class="m2m-api-key-row">${m2mCompactField(keys[0],'Felhasználónév','m2m-api-key-first')}<span class="m2m-api-key-separator" aria-hidden="true">-</span>${m2mCompactField(keys[1],'Felhasználó jelszó')}<span class="m2m-api-key-separator" aria-hidden="true">-</span>${m2mCompactField(keys[2],'Aláírókulcs első fele')}<span class="m2m-api-key-separator" aria-hidden="true">-</span>${m2mCompactField(keys[3],'Nonce')}</div><div class="m2m-connection-subtitle">Kliens azonosítás</div><div class="m2m-client-row">${m2mCompactField(keys[4],'Kliens ID')}${m2mCompactField(keys[5],'Kliens secret')}</div></section>`;}
function listMarkup(){return categories().map(category=>{const data=visibleItems().filter(x=>x.category===category);const m2mKeys=new Set(['nav.m2m.auth.username','nav.m2m.auth.password','nav.m2m.signature.key-first-part','nav.m2m.signature.nonce','nav.m2m.auth.client-id','nav.m2m.auth.client-secret']);const regularData=category==='M2M'?data.filter(x=>!m2mKeys.has(x.key)):data;const connectionCard=category==='M2M'?m2mConnectionCardMarkup():'';let extra='';if(category==='HALOZAT')extra=proxyManagerMarkup();if(category==='TANUSITVANY')extra=certificateManagerMarkup();return `<section class="configuration-list-category" id="configuration-category-${esc(category)}" data-category-block="${esc(category)}"><div class="configuration-list-category-head"><h2>${esc(labels[category]||category)}</h2><span>${data.length} beállítás</span></div>${connectionCard}${regularData.length?listTable(regularData):''}${extra}</section>`;}).join('');}
    /**
   * Megjeleníti vagy újrarendereli a render content állapotát a felhasználói felületen.
   *
   * <p>A függvény mellékhatása lehet DOM- vagy runtime-state módosítás; a hívó a visszatérési értéket és az aszinkron befejeződést a konkrét hívási kontextus szerint kezeli.</p>
   */
function renderContent(){const data=visibleItems().filter(x=>x.category===activeCategory);if(!visibleItems().length){$('configurationContent').innerHTML=`<div class="configuration-empty-state"><h2>Nincs hiányzó beállítás</h2><p>A jelenlegi szűrés szerint minden konfigurációs kulcshoz tartozik kitöltött érték.</p></div>`;updateSummary();return;}if(viewMode==='list'){$('configurationContent').innerHTML=`<div class="configuration-section-head"><div><h2>Összes konfiguráció</h2><p>${advanced?'Minden technikai paraméter látható.':'Az egyszerű módban elérhető beállítások.'} A lista nézet ugyanazokat a funkcionális modulokat tartalmazza, mint a kártya nézet; csak az elrendezés változik.</p></div></div><div class="configuration-all-list">${listMarkup()}</div>`;updateSummary();loadDedicatedProxySettings().catch(e=>show(e.message,'error'));loadCertificates().catch(e=>show(e.message,'error'));focusPendingItem();focusM2mConnectionCard();return;}let extra='';if(activeCategory==='HALOZAT')extra=proxyManagerMarkup();if(activeCategory==='TANUSITVANY')extra=certificateManagerMarkup();const m2mKeys=new Set(['nav.m2m.auth.username','nav.m2m.auth.password','nav.m2m.signature.key-first-part','nav.m2m.signature.nonce','nav.m2m.auth.client-id','nav.m2m.auth.client-secret']);const regularData=activeCategory==='M2M'?data.filter(x=>!m2mKeys.has(x.key)):data;const m2mCard=activeCategory==='M2M'?m2mConnectionCardMarkup():'';$('configurationContent').innerHTML=`<div class="configuration-section-head"><div><h2>${esc(labels[activeCategory]||activeCategory)}</h2><p>${advanced?'Minden technikai paraméter látható.':'A leggyakrabban szükséges beállítások.'} Kártyás szerkesztőnézet.</p></div></div>${m2mCard}<div class="configuration-grid">${regularData.map(cardMarkup).join('')}</div>${extra}`;updateSummary();if(activeCategory==='HALOZAT')loadDedicatedProxySettings().catch(e=>show(e.message,'error'));if(activeCategory==='TANUSITVANY')loadCertificates();focusPendingItem();focusM2mConnectionCard();}
    /**
...
function focusM2mConnectionCard(){if(location.hash!=='#configuration-m2m-connection')return;const card=$('configuration-m2m-connection');if(!card)return;setTimeout(()=>{card.scrollIntoView({behavior:'smooth',block:'center'});card.classList.add('configuration-target-highlight');setTimeout(()=>card.classList.remove('configuration-target-highlight'),1800);},80);}
    /**
...
function activateSensitiveInput(event){const el=event.target.closest('[data-key][data-sensitive="true"]');if(!el)return;el.readOnly=false;sensitiveTouched.add(el.dataset.key);}
  $('configurationContent').addEventListener('paste',e=>{const username=e.target.closest('[data-key="nav.m2m.auth.username"]');if(!username)return;const pasted=e.clipboardData?.getData('text')?.trim()||'';const parts=pasted.split('-');if(parts.length!==4||parts.some(part=>!part.trim()))return;e.preventDefault();const keys=['nav.m2m.auth.username','nav.m2m.auth.password','nav.m2m.signature.key-first-part','nav.m2m.signature.nonce'];keys.forEach((key,index)=>{const input=document.querySelector(`[data-key="${CSS.escape(key)}"]`);if(!input)return;if(input.dataset.sensitive==='true'){input.readOnly=false;sensitiveTouched.add(key);}input.value=parts[index].trim();input.dispatchEvent(new Event('input',{bubbles:true}));});show('Az M2M API kulcs négy mezője kitöltésre került. Mentés előtt ellenőrizd az értékeket.','info');});
